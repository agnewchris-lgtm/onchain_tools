---
name: x-research
description: Research trending topics, accounts, and conversations on X/Twitter using TwitterAPI plus no-credit fallbacks.
---
...
- @KaminoFinance — Kamino (lending)
- @MarinadeFinance — Marinade (staking)
...
## Fallbacks when TwitterAPI credits are unavailable

If TwitterAPI returns `402 Payment Required`, do not treat X research as impossible:

1. Check `xurl` if installed/authenticated (`xurl auth status`, then user/search commands).
2. Fetch `https://x.com/HANDLE` with a browser-like User-Agent; profile pages can include embedded JSON/state with bio, website, follower count, status count, created date, Premium/verified info, and sometimes exact CA text.
3. Cross-check profile metadata mirrors: `https://api.fxtwitter.com/HANDLE` and `https://api.vxtwitter.com/HANDLE`. Treat these as profile evidence only, not full Latest/Top search evidence.
4. For a single tweet/status URL, use `https://api.fxtwitter.com/status/TWEET_ID` first. It often returns text, author stats, metrics, media, quoted tweet, and X Article blocks without TwitterAPI credits. `https://api.vxtwitter.com/HANDLE/status/TWEET_ID` is a smaller backup.
5. For X Articles, the fxtwitter status response may include `tweet.article.content.blocks`; reconstruct the article from non-empty `text` blocks and note `atomic` blocks as image/media placeholders.
6. Unauthenticated `https://x.com/search?...` is usually generic JS/bootstrap HTML and should not be counted as tweet search evidence.
